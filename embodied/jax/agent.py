...
class Agent(embodied.Agent):
    ...
    def __init__(self, model, obs_space, act_space, config, jaxcfg):
        ...
        self.explain_device = flatten(devices[:, self.jaxcfg.policy_devices])
        print('Policy devices:', ', '.join([str(x) for x in self.policy_devices]))
        print('Train devices: ', ', '.join([str(x) for x in self.train_devices]))
        print('Explain devices: ', ', '.join([str(x) for x in self.explain_device]))
        ...
        self.explain_mesh = internal.mesh(
            self.explain_device, self.jaxcfg.train_mesh, ('d', 'f', 't'))
        self.explain_sharding = jax.sharding.NamedSharding(self.explain_mesh, P())
        ...
        self._init_train = transform.apply(
            ...
            **shared_kwargs)
        self._init_explain = transform.apply(nj.pure(self.model.init_explain), self.train_mesh,
                                             (self.train_mirrored, self.train_mirrored), (self.train_mirrored,),
                                             ar, single_output=True, static_argnums=(2,),
                                             **shared_kwargs)
        ...
        self._train = transform.apply(
            ...
            **shared_kwargs)
        self._explain = transform.apply(
            nj.pure(self.model.explain), self.train_mesh,
            (tm, tm, tm, tm,),
            (tm, tm, tm),
            ar,
            static_argnums=(4),
    ...
    @elements.timer.section('jaxagent_explain')
    def explain(self, carry, data, horizon: int = 1):
        data = internal.device_put(data, self.explain_sharding)
        seed = data.pop('seed')
        assert sorted(data.keys()) == sorted(self.spaces.keys()), (
            sorted(data.keys()), sorted(self.spaces.keys()))

        dec_carry, img_recon, acts = self._explain(self.params, seed, carry, data, horizon)
        explanation = self._take_outs(internal.fetch_async(img_recon))
        actions = self._take_outs(internal.fetch_async(acts))
        return carry, explanation, actions

    @elements.timer.section('jaxagent_explain')
    def dec(self, seed, data):
        data = internal.device_put(data,
                                   self.explain_sharding)
        dec_carry, entries, obsrecons = self._explain_dec(self.params, seed, data)
        recon = self._take_outs(internal.fetch_async(obsrecons))
        return dec_carry, entries, recon
